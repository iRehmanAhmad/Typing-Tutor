                ...
                <div className="w-64 bg-bg-secondary border border-border rounded-3xl p-6 flex flex-col gap-3 h-fit sticky top-8">
                    ...
                    {modules.map(mod => (
                        ...
                        >
                            ...
                            <span className="font-bold uppercase tracking-widest text-[11px]">{mod.name}</span>
                        </button>
                    ))}

                    <div className="mt-auto p-4 bg-green-500/10 border border-green-500/20 rounded-2xl">
                        <p className="text-[11px] font-black text-green-500 uppercase tracking-widest text-center">System Status: Optimal</p>
                    </div>
                </div>
                        ...
                        {activeModule === 'overview' && (
                            ...
                            >
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                    {[
                                        ...
                                        <div key={i} className="bg-bg-primary/50 p-6 rounded-2xl border border-border group hover:border-accent transition-all">
                                            <p className="text-[11px] font-black text-text-muted uppercase mb-2">{s.label}</p>
                                ...
                                <div className="space-y-4">
                                    <h3 className="text-[11px] font-black text-text-muted uppercase tracking-[0.3em]">Real-time Engagement Stream</h3>
                                    <div className="grid gap-2">
                                        {recentLogs.map((log, i) => (
                                            <div key={log.id} className="bg-bg-primary/20 border border-border/40 p-3 rounded-xl flex justify-between items-center text-[11px]">
                                        ...
                                        {recentLogs.length === 0 && (
                                            <div className="h-20 flex items-center justify-center border border-dashed border-border rounded-xl opacity-20 text-[11px] uppercase font-black tracking-widest">
                        ...
                        {activeModule === 'ads' && (
                            ...
                            >
                                <div className="flex justify-between items-end">
                                    ...
                                    <button className="px-4 py-2 rounded-lg bg-accent text-white font-bold text-[11px] uppercase tracking-widest">Connect Provider</button>
                                </div>
                                        ...
                                        <div key={unit.id} className="bg-bg-primary/60 p-4 rounded-xl border border-border flex justify-between items-center group hover:bg-bg-primary transition-all">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[11px] text-text-muted font-black uppercase tracking-wider">{unit.provider}</p>
                                                </div>
                                            </div>
                                            <div className="flex gap-2">
                                                <button className="px-3 py-1.5 rounded-lg bg-bg-tertiary text-[11px] font-black uppercase text-text-muted hover:text-text-primary transition-colors">Configure</button>
                                                <button
                                                    onClick={() => toggleAd(unit.id)}
                                                    className={`px-3 py-1.5 rounded-lg text-[11px] font-black uppercase transition-all ${unit.active ? 'bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white' : 'bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white'}`}
                                                >
                            ...
                            >
                                ...
                                <div className="space-y-4">
                                    <div className="bg-bg-primary/50 p-6 rounded-2xl border border-border space-y-4">
                                        <label className="block text-[11px] font-black text-text-muted uppercase tracking-widest">System Alert Banner</label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="bg-bg-primary/50 p-6 rounded-2xl border border-border space-y-4">
                                        <label className="block text-[11px] font-black text-text-muted uppercase tracking-widest">Tactical Tip of the Day</label>
                                        <textarea
