            ...
            <div className="flex bg-bg-primary/50 border-b border-border p-1 gap-1">
                {tabs.map(tab => (
                    <button
                        ...
                        className={`
                            flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl font-black text-[11px] uppercase tracking-widest transition-all
                            ${activeTab === tab.id
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-bg-primary/30">
                            <th className="px-4 py-2 text-[11px] font-black text-text-muted uppercase tracking-[0.2em] border-b border-border w-12 text-center">#</th>
                            <th className="px-4 py-2 text-[11px] font-black text-text-muted uppercase tracking-[0.2em] border-b border-border">Name</th>
                            <th className="px-4 py-2 text-[11px] font-black text-text-muted uppercase tracking-[0.2em] border-b border-border">Time</th>
                            <th className="px-4 py-2 text-[11px] font-black text-text-muted uppercase tracking-[0.2em] border-b border-border">Typed</th>
                            <th className="px-4 py-2 text-[11px] font-black text-text-muted uppercase tracking-[0.2em] border-b border-border w-20">Accuracy</th>
                            <th className="px-4 py-2 text-[11px] font-black text-text-muted uppercase tracking-[0.2em] border-b border-border text-accent">WPM</th>
                            <th className="px-4 py-2 text-[11px] font-black text-text-muted uppercase tracking-[0.2em] border-b border-border text-accent">CPM</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-border/10">
                        <AnimatePresence>
                            {getActiveData().length > 0 ? (
                                getActiveData().map((result, idx) => (
                                    ...
                                    >
                                        <td className="px-4 py-1.5 text-center relative overflow-hidden">
                                            ...
                                            {idx > 2 && <span className="font-black text-text-muted text-[11px] group-hover:text-text-primary transition-colors">{idx + 1}</span>}
                                        </td>
                                        <td className="px-4 py-1.5">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-6 h-6 rounded-full flex items-center justify-center font-black text-[11px] border shadow-xs
                                        ...
                                        <td className="px-4 py-1.5 text-[11px] font-medium text-text-muted">{result.time}</td>
                                        <td className="px-4 py-1.5 text-[11px] font-black text-text-secondary">
                                            {result.words}<span className="text-text-muted opacity-30 font-medium mx-0.5">/</span>{result.chars}
                                        </td>
                                        <td className={`px-4 py-1.5 text-[11px] font-black ${result.accuracy >= 98 ? 'text-green-500' : 'text-text-primary'}`}>
                                            {result.accuracy}%
                                        </td>
                                ...
                                <tr className="h-20">
                                    <td colSpan="8" className="text-center text-[11px] font-black uppercase tracking-widest opacity-20">
                                        Offline
                                    </td>
                                </tr>
                            )}
