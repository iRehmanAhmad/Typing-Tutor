...
const SpeedCircuit = ({ onAbort, isActive = true }) => {
    ...
    return (
        <motion.div animate={screenShake ? { x: [-2, 2, -2, 2, 0], y: [-1, 1, -1, 1, 0] } : {}} transition={{ duration: 0.2 }} className="h-full flex flex-col gap-6 animate-in fade-in zoom-in-95">
            <div className="flex justify-between items-center bg-bg-secondary/50 p-4 rounded-2xl border border-border">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] font-bold text-text-muted uppercase tracking-wider">{gameState === 'briefing' ? 'System Check...' : gameState === 'finished' ? 'Operation Complete' : 'Extraction in Progress'}</p>
                    </div>
                </div>
                <div className="text-right">
                    {gameState === 'briefing' ? <span className="text-4xl font-black text-accent animate-pulse">{countdown}</span> : (
                        <div className="flex gap-6">
                            <div className="text-right"><p className="text-[11px] font-black text-text-muted uppercase">Rank</p><p className="text-xl font-black text-text-primary">{raceResult ? `#${raceResult}` : '...'}</p></div>
                            <div className="text-right"><p className="text-[11px] font-black text-text-muted uppercase">Velocity</p><div className="flex items-baseline gap-1"><p className={`text-xl font-black transition-all ${currentWPM > 80 ? 'text-yellow-400 drop-shadow-[0_0_8px_#fac904]' : 'text-accent'}`}>{currentWPM}</p><span className="text-xs text-text-muted font-black uppercase">WPM</span></div></div>
                        </div>
                    )}
                </div>
            </div>
            <div className="bg-bg-secondary border border-border rounded-3xl p-6 space-y-4 relative overflow-hidden">
                ...
                <div className="relative h-8 bg-bg-tertiary rounded-xl overflow-visible flex items-center px-2">
                    <motion.div className="absolute top-1/2 -translate-y-[62%] left-0 z-20 drop-shadow-lg flex flex-col items-center" animate={{ left: `${playerProgress}%` }} transition={{ type: 'spring', stiffness: 100, damping: 20 }}>
                        ...
                        <div className="absolute -top-8 text-[11px] font-black uppercase text-accent bg-bg-primary px-1 rounded shadow-sm whitespace-nowrap border border-accent/20">YOU</div>
                    </motion.div>
            ...
            <div className="flex-1 relative">
                {gameState === 'finished' && (
                    <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-bg-primary/95 backdrop-blur-xl rounded-3xl space-y-6 border border-border/50">
                        ...
                        <div className="grid grid-cols-2 gap-4 w-full max-w-sm">
                            <div className="bg-bg-tertiary p-4 rounded-2xl border border-border text-center"><p className="text-[11px] font-black text-text-muted uppercase">Avg Velocity</p><p className="text-2xl font-black text-text-primary">{currentWPM} <span className="text-xs uppercase">WPM</span></p></div>
                            <div className="bg-bg-tertiary p-4 rounded-2xl border border-border text-center"><p className="text-[11px] font-black text-text-muted uppercase">Accuracy</p><p className="text-2xl font-black text-text-primary">{accuracy}%</p></div>
                        </div>
                ...
                <TypingArena text={raceText} currentIndex={currentIndex} accuracyMap={accuracyMap} handleKey={handleRaceInput} isRunning={gameState === 'racing'} isFocusMode={gameState === 'racing'} currentWPM={currentWPM} isActive={isActive} />
            </div>
        </motion.div>
    );
};
