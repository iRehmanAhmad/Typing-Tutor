...
const GameCard = ({ title, description, icon, color, onClick, isLocked, tag }) => (
    ...
    >
        <div className="flex justify-between items-start mb-4">
            ...
            {tag && (
                <span className="px-2 py-1 rounded-md bg-bg-tertiary text-[11px] font-black uppercase tracking-wider text-text-muted border border-border/50">
                    {tag}
                </span>
            )}
        </div>
        ...
        <div className="mt-auto flex items-center gap-2 text-accent opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-2 group-hover:translate-y-0">
            <span className="text-[11px] font-black uppercase tracking-widest">Play Game</span>
            <span>→</span>
        </div>
    </motion.button>
);

import SpeedCircuit from '../features/games/SpeedCircuit';
import ThreatNeutralization from '../features/games/ThreatNeutralization';
import SEO from '../layouts/SEO';

// --- Main View ---

const GamesView = ({ isVisible = true }) => {
    ...
    return (
        <div className="max-w-7xl mx-auto min-h-[80vh] flex flex-col">
            ...
            {/* Active Game Modes */}
            {mode === 'active_race' && (
                <SpeedCircuit onAbort={handleBack} isActive={isVisible} />
            )}

            {mode === 'active_survival' && (
                <ThreatNeutralization onAbort={handleBack} isActive={isVisible} />
            )}
        </div>
    );
};
