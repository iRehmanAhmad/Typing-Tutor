...
const VerifyCertificate = ({ certId }) => {
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 md:p-8 relative overflow-hidden">
            ...
            >
                ...
                <div className="flex flex-col items-center text-center space-y-8">
                    ...
                    <div className="space-y-2">
                        <p className="text-[11px] font-black text-accent uppercase tracking-[0.5em]">Typing Master Operations</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full pt-8">
                        {[
                            ...
                            <div key={i} className="bg-bg-primary/50 p-4 rounded-2xl border border-border">
                                <p className="text-[11px] font-black text-text-muted uppercase tracking-widest mb-1">{s.label}</p>
                                <p className="text-xl font-black text-text-primary">{s.val}<small className="text-[11px] ml-0.5 opacity-40">{s.unit}</small></p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="w-full flex flex-col md:flex-row justify-between items-center gap-6 pt-12 border-t border-border mt-8">
                        <div className="text-left space-y-1">
                            <p className="text-[11px] font-black text-text-muted uppercase tracking-widest">Digital ID Authentication</p>
                            <p className="text-xs font-mono text-accent">{certId}</p>
                        </div>
                        <div className="text-right space-y-1">
                            <p className="text-[11px] font-black text-text-muted uppercase tracking-widest">Issue Date</p>
            ...
            <p className="mt-8 text-[11px] font-black text-text-muted uppercase tracking-[0.2em] opacity-40">Typing Master Digital Verification Protocol v1.0.4</p>
        </div>
    );
};
