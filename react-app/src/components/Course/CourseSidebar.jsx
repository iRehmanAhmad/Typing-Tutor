        ...
        <div className="bg-bg-secondary/50 border border-border rounded-3xl h-[calc(100vh-180px)] overflow-y-auto custom-scrollbar shadow-2xl backdrop-blur-md flex flex-col">
            ...
            <div className="p-5 border-b border-border/50 sticky top-0 bg-bg-secondary/95 z-10 backdrop-blur-xl shrink-0">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[9px] font-black text-text-muted/50 border border-white/5 px-2 py-0.5 rounded uppercase tracking-tighter">Verified</span>
                </div>
                    ...
                    return (
                        <div key={lesson.id} className="space-y-4 relative">
                            ...
                            <div className="pl-10 relative">
                                ...
                                `}>
                                    {isAllCompleted ? (
                                        <span className="text-bg-primary text-[8px] font-black">✓</span>
                                    ) : (
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center justify-between">
                                        <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${isUnlocked ? 'text-accent' : 'text-text-muted opacity-50'}`}>
                                            Sequence {lesson.id}
                                        </span>
                                        {!isUnlocked && (
                                            <span className="text-[8px] font-black text-white/20 uppercase tracking-widest bg-white/5 px-2 rounded">Locked</span>
                                        )}
                                    </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-col min-w-0">
                                                <span className={`text-[10px] font-bold truncate uppercase tracking-wide leading-none ${isActive ? 'text-accent' : isUnlocked ? 'text-text-secondary group-hover:text-text-primary' : 'text-text-muted'}`}>
                                                    {sub.title}
                                                </span>
                                            </div>
                                            ...
                                            {isActive && (
                                                <div className="absolute right-3 flex items-center gap-1">
                                                    <span className="text-[8px] font-black text-accent uppercase tracking-tighter animate-pulse">Engaged</span>
                                                </div>
                                            )}
                                        </button>
                                    );
