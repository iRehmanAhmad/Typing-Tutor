...
const SpeedCircuit = ({ onAbort }) => {
    ...
    const [raceResult, setRaceResult] = useState(null); // '1st', '2nd', etc.
    const [startTime, setStartTime] = useState(null);
    const [elapsedTime, setElapsedTime] = useState(0);
    const [screenShake, setScreenShake] = useState(false);
    const overtookBots = useRef(new Set());

    // Select random text for the race
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setBotProgress(prev => {
                ...
                OPPONENTS.forEach(bot => {
                    ...
                    const charPerSec = (effectiveWPM * 5) / 60;
                    const percentPerTick = (charPerSec / raceText.length) * 100 * 0.1;

                    const current = prev[bot.id] || 0;
                    const next = Math.min(100, current + percentPerTick);
        ...
        return () => clearInterval(interval);
    }, [gameState, raceText.length, playerFinished]);

    // Player Progress & Overtake Effect
    ...
    useEffect(() => {
        ...
        const anyBotFinished = Object.values(botProgress).some(progress => progress >= 100);
        if (anyBotFinished) {
            finishRace();
        }
    ...
    const finishRace = () => {
        ...
        OPPONENTS.forEach(bot => {
            ...
            // If they are equal and both are 100, the bot might have won if its update came first
    ...
    return (
        <motion.div
            ...
            className="h-full flex flex-col gap-6 animate-in fade-in zoom-in-95"
        >
            {/* HUD Header */}
            <div className="flex justify-between items-center bg-bg-secondary/50 p-4 rounded-2xl border border-border">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-text-muted uppercase tracking-wider">
                            {gameState === 'briefing' ? 'System Check...' : gameState === 'finished' ? 'Operation Complete' : 'Extraction in Progress'}
                        </p>
                    </div>
                </div>
                ...
                <div className="text-right">
                    {gameState === 'briefing' ? (
                        <span className="text-4xl font-black text-accent animate-pulse">{countdown}</span>
                    ) : (
                        <div className="flex gap-6">
                            <div className="text-right">
                                <p className="text-[10px] font-black text-text-muted uppercase">Rank</p>
                                <p className="text-xl font-black text-text-primary">{raceResult ? `#${raceResult}` : '...'}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-[10px] font-black text-text-muted uppercase">Velocity</p>
                                <div className="flex items-baseline gap-1">
                                    <p className={`text-xl font-black transition-all ${currentWPM > 80 ? 'text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]' : 'text-accent'}`}>
                                        {currentWPM}
                                    </p>
                                    <span className="text-xs text-text-muted font-black uppercase">WPM</span>
                                </div>
                            </div>
                        </div>
                    )}
            ...
            <div className="bg-bg-secondary border border-border rounded-3xl p-6 space-y-4 relative overflow-hidden">
                ...
                <div className="relative h-8 bg-bg-tertiary rounded-xl overflow-visible flex items-center px-2">
                    <motion.div
                        className="absolute top-1/2 -translate-y-[62%] left-0 z-20 drop-shadow-lg flex flex-col items-center"
                        ...
                        <AnimatePresence>
                            {currentWPM > 40 && (
                                <motion.div
                                    ...
                                    animate={{
                                        opacity: Math.min(0.6, (currentWPM - 40) / 100),
                                        scaleX: 1 + (currentWPM / 100),
                                        x: -40
                                    }}
                                    exit={{ opacity: 0 }}
                                    className="absolute right-full top-1/2 -translate-y-1/2 h-4 w-20 bg-gradient-to-r from-transparent to-cyan-400 rounded-full blur-sm pointer-events-none origin-right"
                                />
                            )}
                        </AnimatePresence>
                    ...
                    <div className="w-full h-1.5 bg-border/50 rounded-full overflow-hidden">
                        <motion.div className="h-full bg-accent relative" style={{ width: `${playerProgress}%` }}>
                ...
                {OPPONENTS.map(bot => (
                    <div key={bot.id} className="relative h-6 bg-bg-tertiary/50 rounded-lg overflow-visible flex items-center px-2 opacity-80 mt-6">
                        <motion.div
                            className="absolute top-1/2 -translate-y-[58%] left-0 z-10 grayscale hover:grayscale-0 transition-all"
                            animate={{ left: `${botProgress[bot.id] || 0}%` }}
                        >
                            <span className="text-4xl inline-block -scale-x-100">{bot.icon}</span>
                        </motion.div>
                        <div className="w-full h-1 bg-border/30 rounded-full overflow-hidden">
                            <motion.div className={`h-full ${bot.bg}`} style={{ width: `${botProgress[bot.id] || 0}%` }} />
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="flex-1 relative">
                {gameState === 'finished' ? (
                    <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-bg-primary/95 backdrop-blur-xl rounded-3xl space-y-6 border border-border/50">
                        ...
                        >
                            <h1 className="text-5xl font-black italic uppercase text-accent tracking-tighter drop-shadow-[0_0_20px_rgba(var(--accent-rgb),0.4)]">
                        ...
                        <div className="grid grid-cols-2 gap-4 w-full max-w-sm">
                            <div className="bg-bg-tertiary p-4 rounded-2xl border border-border text-center">
                                <p className="text-[10px] font-black text-text-muted uppercase">Avg Velocity</p>
                                <p className="text-2xl font-black text-text-primary">{currentWPM} <span className="text-xs uppercase">WPM</span></p>
                            </div>
                            <div className="bg-bg-tertiary p-4 rounded-2xl border border-border text-center">
                                <p className="text-[10px] font-black text-text-muted uppercase">Accuracy</p>
                                <p className="text-2xl font-black text-text-primary">{accuracy}%</p>
                            </div>
                        </div>

                        <div className="flex gap-4">
                            <button onClick={onAbort} className="px-8 py-3 rounded-xl bg-bg-secondary text-text-primary font-bold uppercase hover:bg-bg-tertiary transition-all border border-border">
                                Return to Base
                            </button>
                            <button onClick={handleReset} className="px-8 py-3 rounded-xl bg-accent text-background font-black uppercase hover:shadow-[0_0_20px_rgba(var(--accent-rgb),0.4)] transition-all">
                                Re-Deploy
                            </button>
                        </div>
                    </div>
